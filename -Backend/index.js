...
app.get("/getlead/:vendorId", function(req, res) {
  ...
  Report.find({
    "visitors.total_time": { $gt: 0.5 },
    vendor_id: Number(req.params.vendorId)
  }).then(list => {
    res.json({
      message: "Report Found",
      result: list.length ? list[0].visitors : []
    });
  });
});
...
app.get("/createreport", async function(req, res) {
  ...
  let ven = await Vendors.find()
    .toArray()
    .then(data => {
      vendorData = data;
    });
  let rfid = await rfid_tags
    .find()
    .toArray()
    .then(data => {
      rfidData = data;
      ...
      newVendor
        ...
        .then(async vendor => {
          ...
          while ((custDocument = await custCursor.next())) {
            ...
            console.log(cardNum);
            let rfidCursor = await rfid_tags.find({
              id: cardNum,
              reader: readerNum
            });
            let rfidDocument;
...
app.post("/login", function(req, res) {
  ...
  Organizer.findOne({ email: req.body.email }).then(organizer => {
    if (!organizer) {
      ...
      Bcrypt.compare(req.body.password, organizer.password)
        .then(isMatch => {
          if (isMatch) {
...
app.post("/deleteEvent", function(req, res) {
  ...
  Organizer.findOneAndDelete({ email: req.body.email })
    .then(organizer => {
      console.log("orggggggg" + organizer);
      Vendor.deleteMany({ organizer_id: emailid })
        .then(vendor => {
    ...
    if (vendor) {
      ...
      vendor
        ...
        .then(() => {
          ...
          res
            .status(200)
            .json({ auth: true, message: "Vendor added successfully" });
        })
        .catch(err => {
          console.log("Error insereting record: " + err);
        });
...
app.post("/deleteVendor", function(req, res) {
  Vendor.findOneAndDelete({ email: req.body.email })
    .then(vendor => {
      if (!vendor) {
...
app.post("/addUser", function(req, res) {
  User.findOne({ email: req.body.email }).then(user => {
    if (user) {
      return res
        .status(500)
        .json({ message: "User with given email id already exists" });
    } else {
      Bcrypt.genSalt(10, (err, salt) => {
        if (err) throw err;
        Bcrypt.hash(req.body.password, salt, (err, hash) => {
...
app.get("/report/topten/:email", function(req, res) {
  Report.find({ organizer_id: req.params.email, vendor_type: "Stall" })
    .sort({ visitors: -1 })
    .limit(5)
    .then(report => {
      if (!report.length) {
...
app.get("/report/toptenspeakers/:email", function(req, res) {
  Report.find({ organizer_id: req.params.email, vendor_type: "Speaker" })
    .sort({ visitors: -1 })
    .limit(5)
    .then(report => {
      if (!report.length) {
...
app.get("/usersAttended/:email", function(req, res) {
  ...
  User.find({ organizer_id: req.params.email, rfid_id: { $ne: " " } }).then(
    report => {
      if (!report.length) {
        console.log("no report");
        return res.json({ message: "No report Found", result: "" });
